...
import harness_orchestrator.release_publish_handoff_acceptance as acceptance_module
...
class ReleasePublishHandoffAcceptanceTests(unittest.TestCase):
    def test_happy_path_accepts_mapping_dataclass_and_to_dict_verification(self) -> None:
        ...
        for result in results:
            with self.subTest(result=result):
                self.assertIsInstance(result, ReleasePublishHandoffAcceptance)
    ...
    def test_requires_passed_empty_blockers_matching_run_work_safe_ids_and_digest_prefix(self) -> None:
        ...
        cases = (
            ("passed", False, "release-publish-handoff-verification-not-passed"),
            ("blockers", ("operator-review-open",), "source-blockers-present"),
            ("run_id", "run-999", "release-publish-handoff-acceptance-verification-run-id-mismatch"),
            ("work_id", "work-999", "release-publish-handoff-acceptance-verification-work-id-mismatch"),
            ("dependency_id", " release-publish-handoff-package:work-001:bad", "unsafe-dependency-id"),
            ("event_id", "", "missing-event-id"),
            ("package_digest", "f" * 63, "invalid-package-digest"),
            ("package_digest_prefix", "0" * 12, "release-publish-handoff-acceptance-package-digest-prefix-mismatch"),
        )
    ...
    def test_deterministic_ids_optional_expected_values_and_receipt_summary_are_verified(self) -> None:
        base = self._verification().to_dict()
        package_digest = base["package_digest"]
        dependency_id = f"release-publish-handoff-package:work-001:{package_digest[:16]}"
        event_id = f"release-publish-handoff-package-recorded:work-001:{package_digest[:16]}"
        wrong_dependency = copy.deepcopy(base)
        wrong_dependency["dependency_id"] = "release-publish-handoff-package:work-001:ffffffffffffffff"
        wrong_event = copy.deepcopy(base)
        wrong_event["event_id"] = "release-publish-handoff-package-recorded:work-001:ffffffffffffffff"
        wrong_summary = copy.deepcopy(base)
        wrong_summary["receipt_summary"]["package_digest_prefix"] = "0" * 12
        ...
        self.assertIn(
            "release-publish-handoff-acceptance-dependency-id-mismatch",
            self._evaluate(wrong_dependency).blockers,
        )
        self.assertIn(
            "release-publish-handoff-acceptance-event-id-mismatch",
            self._evaluate(wrong_event).blockers,
        )
        self.assertIn(
            "release-publish-handoff-acceptance-receipt-summary-package-digest-prefix-mismatch",
            self._evaluate(wrong_summary).blockers,
        )
        self.assertIn(
            "release-publish-handoff-acceptance-receipt-summary-source-blocker-count-mismatch",
        ...
        self.assertIn(
            ...
            evaluate_release_publish_handoff_acceptance(
                ...
                run_id="run-001",
                work_id="work-001",
                expected_dependency_id="release-publish-handoff-package:work-001:ffffffffffffffff",
            ).blockers,
        )
        self.assertIn(
            ...
            evaluate_release_publish_handoff_acceptance(
                ...
                run_id="run-001",
                work_id="work-001",
                expected_event_id="release-publish-handoff-package-recorded:work-001:ffffffffffffffff",
            ).blockers,
        )
        self.assertIn(
    ...
    def test_malformed_secret_action_non_string_key_and_unsupported_object_fail_closed(self) -> None:
        ...
        self.assertIn(
            "missing-release-publish-handoff-verification",
            evaluate_release_publish_handoff_acceptance(
                "bad",
                run_id="run-001",
                work_id="work-001",
        ...
        self.assertIn(
            "unsafe-release-publish-handoff-verification-schema",
            self._evaluate(extra).blockers,
        )
        self.assertIn(
            "non-string-key-release-publish-handoff-acceptance",
            self._evaluate(non_string).blockers,
        )
        self.assertIn(
            "unsupported-object-release-publish-handoff-acceptance",
    ...
    def _evaluate(self, payload: object) -> ReleasePublishHandoffAcceptance:
        return evaluate_release_publish_handoff_acceptance(
            payload,
            run_id="run-001",
            work_id="work-001",
        )
    ...
    def _verification(self) -> object:
        ...
        intent = build_release_publish_intent(
            ...
            metadata={"ticket": "HAR-038", "approved": True, "count": 2},
        )
