...
import harness_orchestrator.release_publish_execution_authorization_receipt as receipt_module
...
from tests.test_release_publish_execution_authorization_ledger import (
    ReleasePublishExecutionAuthorizationLedgerTests,
)
...
class ReleasePublishExecutionAuthorizationReceiptTests(unittest.TestCase):
    def test_happy_path_accepts_result_snapshot_dataclass_and_to_dict_sources(self) -> None:
        ...
        authorization_summary = metadata["authorization_summary"]
        expected_dependency_id = (
            f"release-publish-execution-authorization:work-001:{package_digest[:16]}"
        )
        expected_event_id = (
            f"release-publish-execution-authorization-recorded:work-001:{package_digest[:16]}"
        )
        ...
        for verified in results:
            with self.subTest(verified=verified):
                ...
                self.assertEqual(authorization_summary["source_dependency_id"], verified.source_dependency_id)
                self.assertEqual(
                    authorization_summary["completion_source_dependency_id"],
                    verified.completion_source_dependency_id,
                )
                self.assertEqual(
                    authorization_summary["final_authorization_event_id"],
                    verified.final_authorization_event_id,
                )
                self.assertEqual(
                    package_digest[:12],
                    verified.authorization_summary["package_digest_prefix"],
                )
                self.assertEqual(
                    package_digest[:12],
    ...
    def test_requires_no_source_blockers_skips_matching_run_work_and_single_pair(self) -> None:
        ...
        skipped_data["skipped_dependency_ids"] = ("dep-001",)
        ...
        duplicate_data["ledger_snapshot"]["audit_events"] = (
            *duplicate_data["ledger_snapshot"]["audit_events"],
            copy.deepcopy(self._event(duplicate_data)),
        )
        ...
        self.assertIn(
            "release-publish-execution-authorization-receipt-receipt-run-id-mismatch",
            self._verify(run_data).blockers,
        )
        self.assertIn(
            "release-publish-execution-authorization-receipt-dependency-work-id-mismatch",
        ...
        self.assertIn(
            "release-publish-execution-authorization-event-ambiguous",
            self._verify(duplicate_data).blockers,
        )
        self.assertIn(
            "release-publish-execution-authorization-recorded-dependency-id-ambiguous",
    ...
    def test_pair_metadata_ids_digest_canonical_expected_values_and_source_chain(self) -> None:
        ...
        self._dependency(source_chain)["metadata"]["authorization_summary"][
            "completion_source_dependency_id"
        ] = "changed"

        self.assertIn(
            "release-publish-execution-authorization-receipt-dependency-id-mismatch",
            self._verify(dependency_id).blockers,
        )
        self.assertIn(
            "release-publish-execution-authorization-receipt-event-dependency-id-mismatch",
            self._verify(event_dependency).blockers,
        )
        self.assertIn(
            "release-publish-execution-authorization-receipt-package-digest-prefix-mismatch",
            self._verify(prefix).blockers,
        )
        self.assertIn("invalid-package-digest", self._verify(digest_payload).blockers)
        self.assertIn(
            "release-publish-execution-authorization-receipt-canonical-payload-format-mismatch",
            self._verify(canonical).blockers,
        )
        self.assertIn(
            "release-publish-execution-authorization-receipt-canonical-payload-release-publish-execution-authorization-mismatch",
            self._verify(canonical_summary).blockers,
        )
        self.assertIn(
            "release-publish-execution-authorization-receipt-authorization-summary-completion-source-dependency-id-mismatch",
    ...
    def test_status_actor_metadata_schema_summaries_and_source_blocker_count_fail_closed(self) -> None:
        ...
        cases.append((dep_status, "release-publish-execution-authorization-dependency-status-not-authorized"))
        not_required = copy.deepcopy(base)
        self._dependency(not_required)["required"] = False
        cases.append((not_required, "release-publish-execution-authorization-dependency-not-required"))
        event_actor = copy.deepcopy(base)
        self._event(event_actor)["actor"] = "operator"
        cases.append((event_actor, "release-publish-execution-authorization-event-actor-mismatch"))
        ...
        cases.append((event_parity, "release-publish-execution-authorization-receipt-event-source-event-id-mismatch"))
        authorized = copy.deepcopy(base)
        self._dependency(authorized)["metadata"]["authorization_summary"]["authorized"] = False
        cases.append((authorized, "release-publish-execution-authorization-receipt-authorization-summary-authorized-mismatch"))
        receipt_event_id = copy.deepcopy(base)
        self._dependency(receipt_event_id)["metadata"]["receipt_summary"]["event_id"] = "changed"
        cases.append((receipt_event_id, "release-publish-execution-authorization-receipt-receipt-summary-event-id-mismatch"))
        count_bool = copy.deepcopy(base)
        self._dependency(count_bool)["metadata"]["receipt_summary"]["source_blocker_count"] = False
        cases.append((count_bool, "release-publish-execution-authorization-receipt-summary-source-blocker-count-mismatch"))
        count_float = copy.deepcopy(base)
        self._dependency(count_float)["metadata"]["receipt_summary"]["source_blocker_count"] = 0.0
        cases.append((count_float, "release-publish-execution-authorization-receipt-summary-source-blocker-count-mismatch"))
    ...
    def test_malformed_secret_action_non_string_key_unsupported_and_bad_to_dict_fail_closed(self) -> None:
        ...
        self.assertIn(
            "missing-release-publish-execution-authorization-receipt",
            verify_release_publish_execution_authorization_receipt(
                "bad",
                run_id="run-001",
                work_id="work-001",
        ...
        self.assertIn(
            "malformed-release-publish-execution-authorization-receipt",
            verify_release_publish_execution_authorization_receipt(
        ...
        self.assertIn(
            "non-string-key-release-publish-execution-authorization-receipt",
            self._verify(non_string).blockers,
        )
        self.assertIn(
            "unsupported-object-release-publish-execution-authorization-receipt",
    ...
    def test_result_is_frozen_plain_json_safe_and_caller_mutation_safe(self) -> None:
        ...
        with self.assertRaises(TypeError):
            verified.authorization_summary["dependency_id"] = "changed"
    ...
    def _verify(
        ...
        return verify_release_publish_execution_authorization_receipt(
            receipt,
            run_id="run-001",
            work_id="work-001",
        )
    ...
    def _ledger_result(
        ...
        *,
    ...
    ) -> tuple[ReleasePublishExecutionAuthorizationLedgerResult, str]:
        helper = ReleasePublishExecutionAuthorizationLedgerTests()
        ...
        return result, authorization.package_digest
    ...
    def _event(self, receipt: dict[str, object]) -> dict[str, object]:
        return next(
            ...
            if event["event_type"] == "release-publish-execution-authorization-ledger-record"
        )
