...
import harness_orchestrator.release_publish_execution_authorization as authorization_module
...
@dataclass(frozen=True)
class HostileVerificationData(VerificationData):
    def to_dict(self) -> dict[str, object]:
        raise RuntimeError("to_dict failed")


class ReleasePublishExecutionAuthorizationTests(unittest.TestCase):
    def test_happy_path_authorizes_mapping_dataclass_and_to_dict_verification(self) -> None:
        ...
        results = (
            authorize_release_publish_execution(
                ...
                expected_final_authorization_dependency_id=data["final_authorization_dependency_id"],
                expected_final_authorization_event_id=data["final_authorization_event_id"],
            ),
            authorize_release_publish_execution(data, run_id="run-001", work_id="work-001"),
            authorize_release_publish_execution(
        ...
        for result in results:
            with self.subTest(result=result):
                self.assertIsInstance(result, ReleasePublishExecutionAuthorization)
                ...
                self.assertEqual(
                    ...
                    result.authorization_summary["completion_source_event_id"],
                )
                self.assertEqual(
    ...
    def test_failed_blocked_mismatch_schema_ids_digest_and_expected_values_fail_closed(self) -> None:
        ...
        cases = (
            ("passed", False, "release-publish-execution-authorization-verification-not-passed"),
            ("blockers", ("operator-review-open",), "source-blockers-present"),
            ("run_id", "run-999", "release-publish-execution-authorization-verification-run-id-mismatch"),
            ("work_id", "work-999", "release-publish-execution-authorization-verification-work-id-mismatch"),
            ("dependency_id", " release-publish-execution-readiness-acceptance:work-001:bad", "unsafe-dependency-id"),
            ("event_id", "", "missing-event-id"),
            ("package_digest", "A" * 64, "invalid-package-digest"),
            ("package_digest_prefix", "0" * 12, "release-publish-execution-authorization-package-digest-prefix-mismatch"),
        ...
        for key, value, blocker in cases:
            ...
            with self.subTest(key=key):
                self.assertIn(blocker, self._authorize(payload).blockers)
        ...
        extra["extra"] = "bad"
        self.assertIn(
            "unsafe-release-publish-execution-authorization-verification-schema",
            self._authorize(missing).blockers,
        )
        self.assertIn(
            "unsafe-release-publish-execution-authorization-verification-schema",
            self._authorize(extra).blockers,
        )

        expected_mismatch = authorize_release_publish_execution(
            base,
            run_id="run-001",
            work_id="work-001",
            expected_dependency_id="release-publish-execution-readiness-acceptance:work-001:ffffffffffffffff",
            expected_event_id=(
                "release-publish-execution-readiness-acceptance-recorded:work-001:ffffffffffffffff"
            ),
        ...
        self.assertIn("expected-final-authorization-dependency-id-mismatch", expected_mismatch.blockers)
        self.assertIn("expected-final-authorization-event-id-mismatch", expected_mismatch.blockers)
    ...
    def test_deterministic_id_source_chain_and_receipt_summary_parity_are_verified(self) -> None:
        ...
        suffix = digest[:16]
        expected = {
            "dependency_id": f"release-publish-execution-readiness-acceptance:work-001:{suffix}",
            "event_id": f"release-publish-execution-readiness-acceptance-recorded:work-001:{suffix}",
            "source_dependency_id": f"release-publish-execution-readiness:work-001:{suffix}",
            "source_event_id": f"release-publish-execution-readiness-recorded:work-001:{suffix}",
            "completion_source_dependency_id": f"release-publish-handoff-acceptance:work-001:{suffix}",
            "completion_source_event_id": f"release-publish-handoff-acceptance-recorded:work-001:{suffix}",
            "acceptance_source_dependency_id": f"release-publish-handoff-package:work-001:{suffix}",
            "acceptance_source_event_id": f"release-publish-handoff-package-recorded:work-001:{suffix}",
            "final_authorization_dependency_id": f"release-publish-final-authorization:work-001:{suffix}",
            "final_authorization_event_id": f"release-publish-final-authorization-recorded:work-001:{suffix}",
        }
        ...
        for key in expected:
            payload = copy.deepcopy(base)
            payload[key] = "changed"
            cases.append((payload, f"release-publish-execution-authorization-{key.replace('_', '-')}-mismatch"))
        for key in expected:
            payload = copy.deepcopy(base)
            ...
            cases.append(
                (
                    payload,
                    f"release-publish-execution-authorization-receipt-summary-{key.replace('_', '-')}-mismatch",
                )
            )
    ...
    def test_bool_and_float_source_blocker_count_reject(self) -> None:
        for value in (False, 0.0):
            ...
            with self.subTest(value=value):
                self.assertIn(
                    "release-publish-execution-authorization-receipt-summary-source-blocker-count-mismatch",
                    self._authorize(payload).blockers,
                )

    def test_malformed_secret_action_non_string_unsupported_bad_to_dict_and_hostile_fail_closed(self) -> None:
        base = self._verification().to_dict()
        ...
        self.assertIn(
            "missing-release-publish-execution-authorization-verification",
            authorize_release_publish_execution(object(), run_id="run-001", work_id="work-001").blockers,
        )
        self.assertIn(_MALFORMED_FOR_TEST, self._authorize(RaisingToDictVerification()).blockers)
        self.assertIn(_MALFORMED_FOR_TEST, self._authorize(NonMappingToDictVerification()).blockers)
        self.assertIn(_MALFORMED_FOR_TEST, self._authorize(HostileVerificationData(**base)).blockers)
        self.assertIn("non-string-key-release-publish-execution-authorization", self._authorize(non_string).blockers)
        self.assertIn("unsupported-object-release-publish-execution-authorization", self._authorize(unsupported).blockers)
        self.assertIn(
            "unsupported-object-release-publish-execution-authorization",
            self._authorize(bad_value_to_dict).blockers,
        )
        self.assertIn(
            "unsupported-object-release-publish-execution-authorization",
            self._authorize(non_mapping_value).blockers,
        )
    ...
    def test_result_is_frozen_plain_json_safe_and_caller_mutation_safe(self) -> None:
        payload = self._verification().to_dict()
        result = self._authorize(payload)
    ...
    def test_forbidden_source_scan_and_import_boundary(self) -> None:
        ...
        imports = tuple(line for line in source.splitlines() if line.startswith(("import ", "from ")))
        self.assertEqual(
            (
    ...
    def _authorize(self, payload: object) -> ReleasePublishExecutionAuthorization:
        return authorize_release_publish_execution(payload, run_id="run-001", work_id="work-001")
    ...
    def _ledger_result(self) -> tuple[ReleasePublishExecutionReadinessAcceptanceLedgerResult, str]:
        helper = ReleasePublishExecutionReadinessAcceptanceLedgerTests()
        ledger, acceptance = helper._ledger_and_acceptance()
        result = record_release_publish_execution_readiness_acceptance(acceptance, ledger=ledger)
        self.assertEqual((), result.blockers)
        return result, acceptance.package_digest


_MALFORMED_FOR_TEST = "malformed-release-publish-execution-authorization"
