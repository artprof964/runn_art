...
import harness_orchestrator.release_publish_execution_readiness_acceptance_receipt as receipt_module
...
class ReleasePublishExecutionReadinessAcceptanceReceiptTests(unittest.TestCase):
    def test_happy_path_accepts_result_snapshot_dataclass_and_to_dict_sources(self) -> None:
        ...
        expected_dependency_id = (
            f"release-publish-execution-readiness-acceptance:work-001:{package_digest[:16]}"
        )
        expected_event_id = (
            f"release-publish-execution-readiness-acceptance-recorded:work-001:{package_digest[:16]}"
        )
        ...
        for verified in results:
            with self.subTest(verified=verified):
                ...
                self.assertEqual(
                    metadata["acceptance_summary"]["completion_source_dependency_id"],
                    verified.completion_source_dependency_id,
                )
                self.assertEqual(
                    metadata["receipt_summary"]["final_authorization_event_id"],
                    verified.final_authorization_event_id,
                )
                self.assertEqual(
                    package_digest[:12],
                    verified.receipt_summary["package_digest_prefix"],
                )
    ...
    def test_requires_no_source_blockers_skips_matching_run_work_and_single_pair(self) -> None:
        ...
        skipped_data["skipped_package_digests"] = ("a" * 64,)
        ...
        self.assertIn("skipped-package-digests-present", self._verify(skipped_data).blockers)
        self.assertIn(
            "release-publish-execution-readiness-acceptance-receipt-receipt-run-id-mismatch",
            self._verify(run_data).blockers,
        )
        self.assertIn(
            "release-publish-execution-readiness-acceptance-receipt-dependency-work-id-mismatch",
        ...
        self.assertIn(
            "release-publish-execution-readiness-acceptance-event-ambiguous",
            self._verify(duplicate_data).blockers,
        )
        self.assertIn(
            "release-publish-execution-readiness-acceptance-recorded-event-id-ambiguous",
            self._verify(duplicate_recorded_data).blockers,
        )

    def test_pair_metadata_ids_digest_canonical_expected_values_and_source_chain(self) -> None:
        result, digest = self._ledger_result()
        base = result.to_dict()
        dependency_id = copy.deepcopy(base)
        self._dependency(dependency_id)["dependency_id"] = (
            "release-publish-execution-readiness-acceptance:work-001:ffffffffffffffff"
        )
        ...
        canonical_summary = copy.deepcopy(base)
        self._dependency(canonical_summary)["metadata"]["canonical_payload"][
            "execution_readiness_acceptance"
        ]["accepted"] = False
        source_chain = copy.deepcopy(base)
        self._dependency(source_chain)["metadata"]["acceptance_summary"][
            "completion_source_dependency_id"
        ] = "changed"

        self.assertIn(
            "release-publish-execution-readiness-acceptance-receipt-dependency-id-mismatch",
            self._verify(dependency_id).blockers,
        )
        self.assertIn(
            "release-publish-execution-readiness-acceptance-receipt-event-dependency-id-mismatch",
            self._verify(event_dependency).blockers,
        )
        self.assertIn(
            "release-publish-execution-readiness-acceptance-receipt-package-digest-prefix-mismatch",
            self._verify(prefix).blockers,
        )
        self.assertIn("invalid-package-digest", self._verify(digest_payload).blockers)
        self.assertIn(
            "release-publish-execution-readiness-acceptance-receipt-canonical-payload-format-mismatch",
            self._verify(canonical).blockers,
        )
        self.assertIn(
            "release-publish-execution-readiness-acceptance-receipt-canonical-payload-execution-readiness-acceptance-mismatch",
            self._verify(canonical_summary).blockers,
        )
        self.assertIn(
            "release-publish-execution-readiness-acceptance-receipt-acceptance-summary-completion-source-dependency-id-mismatch",
            self._verify(source_chain).blockers,
        )
        self.assertIn(
    ...
    def test_status_actor_metadata_schema_summaries_and_source_blocker_count_fail_closed(self) -> None:
        result, _digest = self._ledger_result()
        base = result.to_dict()
        cases = []
        dep_status = copy.deepcopy(base)
        self._dependency(dep_status)["status"] = "pending"
        cases.append((dep_status, "release-publish-execution-readiness-acceptance-dependency-status-not-accepted"))
        not_required = copy.deepcopy(base)
        self._dependency(not_required)["required"] = False
        cases.append((not_required, "release-publish-execution-readiness-acceptance-dependency-not-required"))
        event_actor = copy.deepcopy(base)
        self._event(event_actor)["actor"] = "operator"
        cases.append((event_actor, "release-publish-execution-readiness-acceptance-event-actor-mismatch"))
        ...
        event_parity = copy.deepcopy(base)
        self._event(event_parity)["metadata"]["source_event_id"] = "changed"
        cases.append((event_parity, "release-publish-execution-readiness-acceptance-receipt-event-source-event-id-mismatch"))
        accepted = copy.deepcopy(base)
        self._dependency(accepted)["metadata"]["acceptance_summary"]["accepted"] = False
        cases.append((accepted, "release-publish-execution-readiness-acceptance-receipt-acceptance-summary-accepted-mismatch"))
        receipt_event_id = copy.deepcopy(base)
        self._dependency(receipt_event_id)["metadata"]["receipt_summary"]["event_id"] = "changed"
        cases.append((receipt_event_id, "release-publish-execution-readiness-acceptance-receipt-receipt-summary-event-id-mismatch"))
        count_bool = copy.deepcopy(base)
        self._dependency(count_bool)["metadata"]["receipt_summary"]["source_blocker_count"] = False
        cases.append((count_bool, "release-publish-execution-readiness-acceptance-receipt-summary-source-blocker-count-mismatch"))
        count_float = copy.deepcopy(base)
        self._dependency(count_float)["metadata"]["receipt_summary"]["source_blocker_count"] = 0.0
        cases.append((count_float, "release-publish-execution-readiness-acceptance-receipt-summary-source-blocker-count-mismatch"))
    ...
    def test_malformed_secret_action_non_string_key_unsupported_and_bad_to_dict_fail_closed(self) -> None:
        ...
        self.assertIn(
            "missing-release-publish-execution-readiness-acceptance-receipt",
            verify_release_publish_execution_readiness_acceptance_receipt(
                "bad",
                run_id="run-001",
                work_id="work-001",
        ...
        self.assertIn(
            "malformed-release-publish-execution-readiness-acceptance-receipt",
            verify_release_publish_execution_readiness_acceptance_receipt(
        ...
        self.assertIn(
            "non-string-key-release-publish-execution-readiness-acceptance-receipt",
            self._verify(non_string).blockers,
        )
        self.assertIn(
            "unsupported-object-release-publish-execution-readiness-acceptance-receipt",
    ...
    def _verify(
        ...
        return verify_release_publish_execution_readiness_acceptance_receipt(
            receipt,
            run_id="run-001",
            work_id="work-001",
        )
    ...
    ) -> tuple[ReleasePublishExecutionReadinessAcceptanceLedgerResult, str]:
    ...
    def _event(self, receipt: dict[str, object]) -> dict[str, object]:
        return next(
            ...
            if event["event_type"] == "release-publish-execution-readiness-acceptance-ledger-record"
        )
