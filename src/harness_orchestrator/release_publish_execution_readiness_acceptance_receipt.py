"""Pure receipt verification for Harness release publish execution-readiness acceptance."""
...
_MALFORMED = "malformed-release-publish-execution-readiness-acceptance-receipt"


@dataclass(frozen=True)
class ReleasePublishExecutionReadinessAcceptanceReceiptVerification:
    """Frozen, JSON-safe execution-readiness acceptance verification result."""
...
def verify_release_publish_execution_readiness_acceptance_receipt(
    ...
    *,
...
) -> ReleasePublishExecutionReadinessAcceptanceReceiptVerification:
    """Validate explicit in-memory CR-HAR-051 result or snapshot data."""
    ...
    if source is None:
        blockers.append("missing-release-publish-execution-readiness-acceptance-receipt")
        source = {}
    ...
    if set(snapshot) != _SNAPSHOT_KEYS:
        blockers.append("unsafe-release-publish-execution-readiness-acceptance-snapshot-schema")
    ...
    if typed_dependencies and not matching_dependencies:
        blockers.append("release-publish-execution-readiness-acceptance-receipt-dependency-work-id-mismatch")
    if typed_events and not matching_events:
        blockers.append("release-publish-execution-readiness-acceptance-receipt-event-work-id-mismatch")
    if len(matching_dependencies) != 1:
        blockers.append(
            "release-publish-execution-readiness-acceptance-dependency-missing"
            if not matching_dependencies
            else "release-publish-execution-readiness-acceptance-dependency-ambiguous"
        )
    if len(matching_events) != 1:
        blockers.append(
            "release-publish-execution-readiness-acceptance-event-missing"
            if not matching_events
            else "release-publish-execution-readiness-acceptance-event-ambiguous"
        )
    ...
    frozen_summary = _freeze_value(receipt_summary) if receipt_summary is not None else None
    return ReleasePublishExecutionReadinessAcceptanceReceiptVerification(
        passed=not deduped,
        blockers=deduped,
        ...
        receipt_summary=frozen_summary if isinstance(frozen_summary, Mapping) else None,
    )
...
def _snapshot_from_source(source: Mapping[str, Any], blockers: list[str]) -> dict[str, Any]:
    ...
    if set(source) != _RESULT_KEYS:
        blockers.append("unsafe-release-publish-execution-readiness-acceptance-result-schema")
...
def _validate_pair(
    *,
    ...
    work_id: str,
    blockers: list[str],
) -> tuple[str, str, str, str, str, str, str, str, str, str, str, Mapping[str, Any] | None]:
    if dependency.get("required") is not True:
        blockers.append("release-publish-execution-readiness-acceptance-dependency-not-required")
    if dependency.get("status") != "accepted":
        blockers.append("release-publish-execution-readiness-acceptance-dependency-status-not-accepted")
    if event.get("status") != "accepted":
        blockers.append("release-publish-execution-readiness-acceptance-event-status-not-accepted")
    if event.get("actor") != "harness":
        blockers.append("release-publish-execution-readiness-acceptance-event-actor-mismatch")
    ...
    if package_digest:
        ...
        _match(dependency_id, f"{_DEPENDENCY_TYPE}:{work_id}:{suffix}", "dependency-id", blockers)
        _match(
            event_id,
            f"release-publish-execution-readiness-acceptance-recorded:{work_id}:{suffix}",
            "event-id",
            blockers,
        )
        _validate_source_chain(dependency_metadata, work_id, suffix, blockers)

    acceptance_summary = _required_mapping(
        dependency_metadata.get("acceptance_summary"),
        "acceptance-summary",
        blockers,
    )
    ...
    _match(
        ...
        acceptance_summary,
        "canonical-payload-execution-readiness-acceptance",
        blockers,
    )
    return (
        dependency_id,
        ...
        str(acceptance_summary.get("completion_source_dependency_id", "")),
        str(acceptance_summary.get("completion_source_event_id", "")),
        str(acceptance_summary.get("acceptance_source_dependency_id", "")),
        str(acceptance_summary.get("acceptance_source_event_id", "")),
        str(acceptance_summary.get("final_authorization_dependency_id", "")),
        str(acceptance_summary.get("final_authorization_event_id", "")),
        receipt_summary,
    )
...
def _validate_acceptance_summary(
    ...
    _validate_summary_common(summary, metadata, work_id, package_digest, "acceptance-summary", blockers)
...
def _validate_receipt_summary(
    ...
    _validate_summary_common(summary, metadata, work_id, package_digest, "receipt-summary", blockers)
...
def _validate_summary_common(
    ...
    _match(summary.get("dependency_id"), expected["execution_dependency_id"], f"{label}-dependency-id", blockers)
    _match(summary.get("event_id"), expected["execution_event_id"], f"{label}-event-id", blockers)
    _match(summary.get("package_digest_prefix"), metadata.get("package_digest_prefix"), f"{label}-package-digest-prefix", blockers)
    _match(summary.get("source_dependency_id"), expected["readiness_dependency_id"], f"{label}-source-dependency-id", blockers)
    _match(summary.get("source_event_id"), expected["readiness_event_id"], f"{label}-source-event-id", blockers)
    _match(summary.get("completion_source_dependency_id"), expected["completion_dependency_id"], f"{label}-completion-source-dependency-id", blockers)
    _match(summary.get("completion_source_event_id"), expected["completion_event_id"], f"{label}-completion-source-event-id", blockers)
    _match(summary.get("acceptance_source_dependency_id"), expected["acceptance_dependency_id"], f"{label}-acceptance-source-dependency-id", blockers)
    _match(summary.get("acceptance_source_event_id"), expected["acceptance_event_id"], f"{label}-acceptance-source-event-id", blockers)
    _match(summary.get("final_authorization_dependency_id"), expected["final_dependency_id"], f"{label}-final-authorization-dependency-id", blockers)
    _match(summary.get("final_authorization_event_id"), expected["final_event_id"], f"{label}-final-authorization-event-id", blockers)
    _match(metadata.get("source_dependency_id"), expected["execution_dependency_id"], f"{label}-metadata-source-dependency-id", blockers)
    _match(metadata.get("source_event_id"), expected["execution_event_id"], f"{label}-metadata-source-event-id", blockers)
...
def _validate_source_chain(
    ...
    expected = _source_ids(work_id, suffix)
    _match(metadata.get("source_dependency_id"), expected["execution_dependency_id"], "source-chain-dependency-id", blockers)
    _match(metadata.get("source_event_id"), expected["execution_event_id"], "source-chain-event-id", blockers)


def _source_ids(work_id: str, suffix: str) -> dict[str, str]:
    return {
        "execution_dependency_id": f"release-publish-execution-readiness:{work_id}:{suffix}",
        "execution_event_id": f"release-publish-execution-readiness-recorded:{work_id}:{suffix}",
        "readiness_dependency_id": f"release-publish-final-authorization:{work_id}:{suffix}",
        "readiness_event_id": f"release-publish-final-authorization-recorded:{work_id}:{suffix}",
        ...
        "acceptance_dependency_id": f"release-publish-handoff-package:{work_id}:{suffix}",
        "acceptance_event_id": f"release-publish-handoff-package-recorded:{work_id}:{suffix}",
        "final_dependency_id": f"release-publish-final-authorization:{work_id}:{suffix}",
        "final_event_id": f"release-publish-final-authorization-recorded:{work_id}:{suffix}",
    }
...
def _validate_recorded_ids(
    *,
    ...
    if len(dependency_ids) != 1:
        blockers.append(
            "release-publish-execution-readiness-acceptance-recorded-dependency-id-missing"
            if not dependency_ids
            else "release-publish-execution-readiness-acceptance-recorded-dependency-id-ambiguous"
        )
    elif dependency_id and dependency_ids[0] != dependency_id:
        blockers.append("release-publish-execution-readiness-acceptance-recorded-dependency-id-mismatch")
    if len(event_ids) != 1:
        blockers.append(
            "release-publish-execution-readiness-acceptance-recorded-event-id-missing"
            if not event_ids
            else "release-publish-execution-readiness-acceptance-recorded-event-id-ambiguous"
        )
    elif event_id and event_ids[0] != event_id:
        blockers.append("release-publish-execution-readiness-acceptance-recorded-event-id-mismatch")
...
def _validated_mapping(value: Mapping[Any, Any], blockers: list[str]) -> dict[str, Any]:
    ...
    for key, item in value.items():
        if not isinstance(key, str):
            blockers.append("non-string-key-release-publish-execution-readiness-acceptance-receipt")
            continue
...
def _plain_value(value: object, blockers: list[str]) -> object:
    ...
    if callable(to_dict):
        try:
            ...
            blockers.append("unsupported-object-release-publish-execution-readiness-acceptance-receipt")
            return None
        if isinstance(mapped, Mapping):
            return _validated_mapping(mapped, blockers)
        blockers.append("unsupported-object-release-publish-execution-readiness-acceptance-receipt")
...
def _match(actual: object, expected: object, name: str, blockers: list[str]) -> None:
    if actual != expected:
        blockers.append(f"release-publish-execution-readiness-acceptance-receipt-{name}-mismatch")
