"""Pure receipt verification for Harness release publish execution handoff readiness."""
...
_ALLOWED_ACTION_KEYS = frozenset(
    {
        ...
        "release_publish_execution_handoff_readiness_ledger",
        "release_publish_execution_handoff_readiness_ledger_record",
        "release_publish_execution_handoff_readiness_ledger_v1",
    }
)
_ACTION_TEXT = (
    "$(",
...
_MALFORMED = "malformed-release-publish-execution-handoff-readiness-receipt"


@dataclass(frozen=True)
class ReleasePublishExecutionHandoffReadinessReceiptVerification:
    """Frozen, JSON-safe handoff readiness ledger receipt verification."""
...
def verify_release_publish_execution_handoff_readiness_receipt(
    ...
    *,
    ...
    """Validate explicit in-memory CR-HAR-057 ledger result or snapshot data."""
    ...
    if source is None:
        blockers.append("missing-release-publish-execution-handoff-readiness-receipt")
        source = {}
    ...
    if set(snapshot) != _SNAPSHOT_KEYS:
        blockers.append("unsafe-release-publish-execution-handoff-readiness-snapshot-schema")
    ...
    matching_events = tuple(event for event in typed_events if event.get("work_id") == expected_work_id)
    if typed_dependencies and not matching_dependencies:
        blockers.append("release-publish-execution-handoff-readiness-receipt-dependency-work-id-mismatch")
    if typed_events and not matching_events:
        blockers.append("release-publish-execution-handoff-readiness-receipt-event-work-id-mismatch")
    if len(matching_dependencies) != 1:
        blockers.append(
            "release-publish-execution-handoff-readiness-dependency-missing"
            if not matching_dependencies
            else "release-publish-execution-handoff-readiness-dependency-ambiguous"
        )
    if len(matching_events) != 1:
        blockers.append(
            "release-publish-execution-handoff-readiness-event-missing"
            if not matching_events
            else "release-publish-execution-handoff-readiness-event-ambiguous"
        )
    ...
    frozen_readiness = _freeze_value(readiness_summary) if readiness_summary is not None else None
    ...
    return ReleasePublishExecutionHandoffReadinessReceiptVerification(
        passed=not deduped,
        blockers=deduped,
        ...
        readiness_summary=frozen_readiness if isinstance(frozen_readiness, Mapping) else None,
...
def _snapshot_from_source(source: Mapping[str, Any], blockers: list[str]) -> dict[str, Any]:
    ...
    if set(source) != _RESULT_KEYS:
        blockers.append("unsafe-release-publish-execution-handoff-readiness-result-schema")
...
def _validate_pair(
    *,
    ...
    work_id: str,
    blockers: list[str],
) -> tuple[str, str, str, str, str, Mapping[str, Any] | None, Mapping[str, Any] | None, Mapping[str, Any] | None]:
    if dependency.get("required") is not True:
        blockers.append("release-publish-execution-handoff-readiness-dependency-not-required")
    if not (type(dependency.get("order")) is int and dependency.get("order") == 140):
        blockers.append("release-publish-execution-handoff-readiness-dependency-order-mismatch")
    if dependency.get("status") != "ready":
        blockers.append("release-publish-execution-handoff-readiness-dependency-status-not-ready")
    if event.get("status") != "ready":
        blockers.append("release-publish-execution-handoff-readiness-event-status-not-ready")
    if event.get("actor") != "harness":
        blockers.append("release-publish-execution-handoff-readiness-event-actor-mismatch")
    ...
    if set(canonical) != {"format", "release_publish_execution_handoff_readiness"}:
        blockers.append("unsafe-canonical-payload-schema")
    _match(canonical.get("format"), _FORMAT, "canonical-payload-format", blockers)
    _match(
        canonical.get("release_publish_execution_handoff_readiness"),
        readiness_summary,
        "canonical-payload-release-publish-execution-handoff-readiness",
        blockers,
    )
    return (
        dependency_id,
...
def _validate_readiness_summary(
    ...
    _match(summary.get("work_id"), expected["work_id"], "readiness-summary-work-id", blockers)
    _match(summary.get("dependency_id"), expected["dependency_id"], "readiness-summary-dependency-id", blockers)
    _match(summary.get("event_id"), expected["event_id"], "readiness-summary-event-id", blockers)
    ...
    _match(summary.get("source_dependency_id"), expected["authorization_dependency_id"], "readiness-summary-source-dependency-id", blockers)
    _match(summary.get("source_event_id"), expected["authorization_event_id"], "readiness-summary-source-event-id", blockers)
    _match(summary.get("authorization_dependency_id"), expected["authorization_dependency_id"], "readiness-summary-authorization-dependency-id", blockers)
    _match(summary.get("authorization_event_id"), expected["authorization_event_id"], "readiness-summary-authorization-event-id", blockers)
    if summary.get("ready") is not True:
        blockers.append("release-publish-execution-handoff-readiness-receipt-readiness-summary-ready-mismatch")
...
def _validate_authorization_summary(
    ...
    _match(summary.get("run_id"), metadata.get("run_id"), "authorization-summary-run-id", blockers)
    _match(summary.get("work_id"), expected["work_id"], "authorization-summary-work-id", blockers)
    ...
    _match(summary.get("package_digest_prefix"), metadata.get("package_digest_prefix"), "authorization-summary-package-digest-prefix", blockers)
    ...
    if summary.get("authorized") is not True:
        blockers.append("release-publish-execution-handoff-readiness-receipt-authorization-summary-authorized-mismatch")
...
def _validate_receipt_summary(
    ...
    _match(summary.get("work_id"), expected["work_id"], "receipt-summary-work-id", blockers)
...
def _validate_recorded_ids(
    *,
    ...
    if len(dependency_ids) != 1:
        blockers.append(
            "release-publish-execution-handoff-readiness-recorded-dependency-id-missing"
            if not dependency_ids
            else "release-publish-execution-handoff-readiness-recorded-dependency-id-ambiguous"
        )
    elif dependency_id and dependency_ids[0] != dependency_id:
        blockers.append("release-publish-execution-handoff-readiness-recorded-dependency-id-mismatch")
    if len(event_ids) != 1:
        blockers.append(
            "release-publish-execution-handoff-readiness-recorded-event-id-missing"
            if not event_ids
            else "release-publish-execution-handoff-readiness-recorded-event-id-ambiguous"
        )
    elif event_id and event_ids[0] != event_id:
        blockers.append("release-publish-execution-handoff-readiness-recorded-event-id-mismatch")
...
def _validated_mapping(value: Mapping[Any, Any], blockers: list[str]) -> dict[str, Any]:
    ...
    for key, item in value.items():
        if not isinstance(key, str):
            blockers.append("non-string-key-release-publish-execution-handoff-readiness-receipt")
            continue
...
def _plain_value(value: object, blockers: list[str]) -> object:
    ...
    if callable(to_dict):
        try:
            ...
            blockers.append("unsupported-object-release-publish-execution-handoff-readiness-receipt")
            return None
        if isinstance(mapped, Mapping):
            return _validated_mapping(mapped, blockers)
        blockers.append("unsupported-object-release-publish-execution-handoff-readiness-receipt")
...
def _match(actual: object, expected: object, name: str, blockers: list[str]) -> None:
    if actual != expected:
        blockers.append(f"release-publish-execution-handoff-readiness-receipt-{name}-mismatch")
