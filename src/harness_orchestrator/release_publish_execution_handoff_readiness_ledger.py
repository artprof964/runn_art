"""Record release publish execution handoff readiness into an explicit ledger."""
...
_FORMAT = "harness-release-publish-execution-handoff-readiness-ledger-v1"
_SOURCE_FORMAT = "harness-release-publish-execution-authorization-ledger-v1"
_DEPENDENCY_TYPE = "release-publish-execution-handoff-readiness"
_EVENT_TYPE = "release-publish-execution-handoff-readiness-ledger-record"
_SOURCE_DEPENDENCY_TYPE = "release-publish-execution-authorization"
_SOURCE_EVENT_TYPE = "release-publish-execution-authorization-ledger-record"
...
_MALFORMED_BLOCKER = "malformed-release-publish-execution-handoff-readiness-ledger-data"


@dataclass(frozen=True)
class ReleasePublishExecutionHandoffReadinessLedgerResult:
    """Plain result from recording execution handoff readiness data."""
...
def record_release_publish_execution_handoff_readinesses(
    readinesses: object,
    *,
    ledger: RunLedger | None,
) -> ReleasePublishExecutionHandoffReadinessLedgerResult:
    """Record already-built CR-HAR-056 readiness data into an injected ledger."""

    if not isinstance(ledger, RunLedger):
        return ReleasePublishExecutionHandoffReadinessLedgerResult(
    ...
    if not items:
        return ReleasePublishExecutionHandoffReadinessLedgerResult(
            blockers=("release-publish-execution-handoff-readiness-empty",),
    ...
    existing_package_digests = _existing_handoff_readiness_package_digests(snapshot)
...
def record_release_publish_execution_handoff_readiness(
    readiness: object,
    *,
    ledger: RunLedger | None,
) -> ReleasePublishExecutionHandoffReadinessLedgerResult:
    """Compatibility wrapper for recording one execution handoff readiness."""

    return record_release_publish_execution_handoff_readinesses(readiness, ledger=ledger)


def _normalize_items(value: object) -> tuple[object, ...]:
    if value is None:
        return ()
    if isinstance(value, ReleasePublishExecutionHandoffReadiness):
        return (value,)
...
def _readiness_from_item(item: object) -> tuple[dict[str, Any] | None, list[str]]:
    ...
    if mapped is None:
        blockers.append("release-publish-execution-handoff-readiness-wrong-type")
        return None, blockers

    if set(mapped) != _READINESS_KEYS:
        blockers.append("unsafe-release-publish-execution-handoff-readiness-schema")
    if mapped.get("ready") is not True:
        blockers.append("release-publish-execution-handoff-readiness-not-ready")
    if mapped.get("status") != "ready":
        blockers.append("release-publish-execution-handoff-readiness-status-not-ready")
    source_blockers = _blocker_tuple(mapped.get("blockers"))
    if source_blockers:
        blockers.append("release-publish-execution-handoff-readiness-blockers-present")
        blockers.extend(
            f"release-publish-execution-handoff-readiness-{blocker}"
    ...
    authorization_dependency_id = _required_text(
        mapped.get("authorization_dependency_id"),
        "authorization-dependency-id",
        blockers,
    )
    authorization_event_id = _required_text(
        mapped.get("authorization_event_id"),
    ...
    authorization_summary = _required_mapping(
        mapped.get("authorization_summary"),
    ...
    expected = _expected_ids(work_id, package_digest)
    ...
    if work_id and package_digest:
        _match(dependency_id, expected["dependency_id"], "dependency-id", blockers)
        _match(event_id, expected["event_id"], "event-id", blockers)
        _match(authorization_dependency_id, expected["authorization_dependency_id"], "authorization-dependency-id", blockers)
        _match(authorization_event_id, expected["authorization_event_id"], "authorization-event-id", blockers)
        _match(source_dependency_id, authorization_dependency_id, "source-dependency-id", blockers)
        _match(source_event_id, authorization_event_id, "source-event-id", blockers)
    ...
    if _contains_secret_like(mapped):
        blockers.append("secret-like-release-publish-execution-handoff-readiness-ledger-data")
    if _contains_action_intent(mapped):
        blockers.append("action-intent-release-publish-execution-handoff-readiness-ledger-data")
...
def _validate_readiness_summary(
    *,
    ...
    _match(summary.get("source_dependency_id"), authorization_dependency_id, "readiness-summary-source-dependency-id", blockers)
    _match(summary.get("source_event_id"), authorization_event_id, "readiness-summary-source-event-id", blockers)
    _match(summary.get("authorization_dependency_id"), authorization_dependency_id, "readiness-summary-authorization-dependency-id", blockers)
    _match(summary.get("authorization_event_id"), authorization_event_id, "readiness-summary-authorization-event-id", blockers)
...
def _validate_authorization_summary(
    *,
    ...
    _match(summary.get("run_id"), run_id, "authorization-summary-run-id", blockers)
    _match(summary.get("work_id"), work_id, "authorization-summary-work-id", blockers)
    _match(summary.get("dependency_id"), expected["acceptance_dependency_id"], "authorization-summary-dependency-id", blockers)
    _match(summary.get("event_id"), expected["acceptance_event_id"], "authorization-summary-event-id", blockers)
    _match(summary.get("package_digest_prefix"), package_digest_prefix, "authorization-summary-package-digest-prefix", blockers)
    _match(summary.get("source_dependency_id"), expected["readiness_dependency_id"], "authorization-summary-source-dependency-id", blockers)
    _match(summary.get("source_event_id"), expected["readiness_event_id"], "authorization-summary-source-event-id", blockers)
    _match(summary.get("completion_source_dependency_id"), expected["completion_dependency_id"], "authorization-summary-completion-source-dependency-id", blockers)
    _match(summary.get("completion_source_event_id"), expected["completion_event_id"], "authorization-summary-completion-source-event-id", blockers)
    _match(summary.get("acceptance_source_dependency_id"), expected["package_dependency_id"], "authorization-summary-acceptance-source-dependency-id", blockers)
    _match(summary.get("acceptance_source_event_id"), expected["package_event_id"], "authorization-summary-acceptance-source-event-id", blockers)
    _match(summary.get("final_authorization_dependency_id"), expected["final_authorization_dependency_id"], "authorization-summary-final-authorization-dependency-id", blockers)
    _match(summary.get("final_authorization_event_id"), expected["final_authorization_event_id"], "authorization-summary-final-authorization-event-id", blockers)
...
def _validate_receipt_summary(
    *,
    ...
    _match(summary.get("dependency_id"), authorization_summary.get("source_dependency_id"), "receipt-summary-dependency-id", blockers)
    _match(summary.get("event_id"), authorization_summary.get("source_event_id"), "receipt-summary-event-id", blockers)
    _match(summary.get("package_digest_prefix"), package_digest_prefix, "receipt-summary-package-digest-prefix", blockers)
    _match(summary.get("source_dependency_id"), expected["final_authorization_dependency_id"], "receipt-summary-source-dependency-id", blockers)
    _match(summary.get("source_event_id"), expected["final_authorization_event_id"], "receipt-summary-source-event-id", blockers)
    _match(summary.get("completion_source_dependency_id"), expected["completion_dependency_id"], "receipt-summary-completion-source-dependency-id", blockers)
    _match(summary.get("completion_source_event_id"), expected["completion_event_id"], "receipt-summary-completion-source-event-id", blockers)
    _match(summary.get("acceptance_source_dependency_id"), expected["package_dependency_id"], "receipt-summary-acceptance-source-dependency-id", blockers)
    _match(summary.get("acceptance_source_event_id"), expected["package_event_id"], "receipt-summary-acceptance-source-event-id", blockers)
    _match(summary.get("final_authorization_dependency_id"), expected["final_authorization_dependency_id"], "receipt-summary-final-authorization-dependency-id", blockers)
    _match(summary.get("final_authorization_event_id"), expected["final_authorization_event_id"], "receipt-summary-final-authorization-event-id", blockers)
...
def _ledger_records(readiness: Mapping[str, Any]) -> tuple[DependencyRecord, AuditEvent]:
    ...
    metadata = {
        "run_id": run_id,
        "source_dependency_id": readiness["authorization_dependency_id"],
        "source_event_id": readiness["authorization_event_id"],
        "package_digest": package_digest,
        "package_digest_prefix": readiness["package_digest_prefix"],
        "readiness_summary": readiness["readiness_summary"],
        "authorization_summary": readiness["authorization_summary"],
        "receipt_summary": readiness["receipt_summary"],
        "canonical_payload": {
            "format": _FORMAT,
            "release_publish_execution_handoff_readiness": readiness["readiness_summary"],
        },
    }
    dependency = DependencyRecord(
        dependency_id=f"{_DEPENDENCY_TYPE}:{work_id}:{suffix}",
        work_id=work_id,
        reference=f"{_DEPENDENCY_TYPE}:{run_id}:{work_id}:{suffix}",
        order=140,
    ...
    event = AuditEvent(
        ...
        status="ready",
        message="Release publish execution handoff readiness recorded in ledger.",
        actor="harness",
        metadata={"dependency_id": dependency.dependency_id, **metadata},
    )
...
def _source_record_blockers(
    ...
    dependencies = [
        ...
        if record.dependency_id == readiness.get("authorization_dependency_id")
    ]
    events = [
        event
        for event in snapshot.audit_events
        if event.event_id == readiness.get("authorization_event_id")
    ]
    if len(dependencies) != 1:
        blockers.append(
            "source-execution-authorization-dependency-missing"
            if not dependencies
            else "source-execution-authorization-dependency-ambiguous"
        )
    if len(events) != 1:
        blockers.append(
            "source-execution-authorization-event-missing"
            if not events
            else "source-execution-authorization-event-ambiguous"
        )
    if len(dependencies) == 1:
...
def _validate_source_dependency(
    ...
    if dependency.dependency_type != _SOURCE_DEPENDENCY_TYPE:
        blockers.append("source-execution-authorization-dependency-type-mismatch")
    if dependency.work_id != readiness["work_id"]:
        blockers.append("source-execution-authorization-dependency-work-id-mismatch")
    if dependency.required is not True:
        blockers.append("source-execution-authorization-dependency-required-mismatch")
    if dependency.status != "authorized":
        blockers.append("source-execution-authorization-dependency-status-mismatch")
...
def _validate_source_event(
    ...
    if event.event_type != _SOURCE_EVENT_TYPE:
        blockers.append("source-execution-authorization-event-type-mismatch")
    if event.work_id != readiness["work_id"]:
        blockers.append("source-execution-authorization-event-work-id-mismatch")
    if event.status != "authorized":
        blockers.append("source-execution-authorization-event-status-mismatch")
    if event.actor != "harness":
        blockers.append("source-execution-authorization-event-actor-mismatch")
    _validate_source_metadata(
        event.metadata,
        readiness,
        "event",
        blockers,
        expected_dependency_id=readiness["authorization_dependency_id"],
    )
...
def _validate_source_metadata(
    ...
    blockers: list[str],
    *,
    expected_dependency_id: str | None = None,
) -> None:
    keys = _SOURCE_EVENT_METADATA_KEYS if expected_dependency_id is not None else _SOURCE_METADATA_KEYS
    if set(metadata) != keys:
        blockers.append(f"source-execution-authorization-{owner}-metadata-schema-mismatch")
    if expected_dependency_id is not None:
        _match_source(metadata.get("dependency_id"), expected_dependency_id, f"{owner}-dependency-id", blockers)
    _match_source(metadata.get("run_id"), readiness["run_id"], f"{owner}-run-id", blockers)
    _match_source(metadata.get("package_digest"), readiness["package_digest"], f"{owner}-package-digest", blockers)
    _match_source(metadata.get("package_digest_prefix"), readiness["package_digest_prefix"], f"{owner}-package-digest-prefix", blockers)
    _match_source(metadata.get("source_dependency_id"), readiness["authorization_summary"]["dependency_id"], f"{owner}-source-dependency-id", blockers)
    _match_source(metadata.get("source_event_id"), readiness["authorization_summary"]["event_id"], f"{owner}-source-event-id", blockers)
    _match_source(metadata.get("authorization_summary"), readiness["authorization_summary"], f"{owner}-authorization-summary", blockers)
    _match_source(metadata.get("receipt_summary"), readiness["receipt_summary"], f"{owner}-receipt-summary", blockers)
    canonical = _required_mapping(metadata.get("canonical_payload"), "source-canonical-payload", blockers)
    _match_source(canonical.get("format"), _SOURCE_FORMAT, f"{owner}-canonical-format", blockers)
    _match_source(
        canonical.get("release_publish_execution_authorization"),
        readiness["authorization_summary"],
        f"{owner}-canonical-authorization",
        blockers,
    )
...
def _duplicate_blockers(
    *,
    ...
    if dependency.dependency_id in seen_dependency_ids:
        blockers.append("release-publish-execution-handoff-readiness-dependency-id-duplicate")
    elif dependency.dependency_id in existing_dependency_ids:
        blockers.append("release-publish-execution-handoff-readiness-dependency-id-already-recorded")
    if event.event_id in seen_event_ids:
        blockers.append("release-publish-execution-handoff-readiness-event-id-duplicate")
    elif event.event_id in existing_event_ids:
        blockers.append("release-publish-execution-handoff-readiness-event-id-already-recorded")
    if package_digest in seen_package_digests:
        blockers.append("release-publish-execution-handoff-readiness-package-digest-duplicate")
    elif package_digest in existing_package_digests:
        blockers.append("release-publish-execution-handoff-readiness-package-digest-already-recorded")
    return tuple(blockers)


def _existing_handoff_readiness_package_digests(snapshot: RunLedgerSnapshot) -> set[str]:
    values: set[str] = set()
    for record in snapshot.dependencies:
    ...
    for key, item in value.items():
        if not isinstance(key, str):
            ...
            if blockers is not None:
                blockers.append("non-string-key-release-publish-execution-handoff-readiness-ledger-data")
            continue
...
def _plain_value(value: object, blockers: list[str] | None = None) -> object:
    ...
    if callable(to_dict):
        try:
            ...
            if blockers is not None:
                blockers.append("unsupported-object-release-publish-execution-handoff-readiness-ledger-data")
            return None
        if isinstance(mapped, Mapping):
...
def _match_source(actual: object, expected: object, name: str, blockers: list[str]) -> None:
    if actual != expected:
        blockers.append(f"source-execution-authorization-{name}-mismatch")


def _expected_ids(work_id: str, package_digest: str) -> dict[str, str]:
    suffix = package_digest[:16]
    return {
