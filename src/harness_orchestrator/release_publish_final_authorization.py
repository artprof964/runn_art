"""Pure final authorization boundary for Harness release publish."""
...
class ReleasePublishFinalAuthorization:
    """Frozen, JSON-safe final publish authorization data."""
...
def authorize_release_publish_final(
    ...
    *,
    ...
    """Authorize only already-passed, explicit CR-HAR-043 verification data."""
    ...
    if verification is None:
        blockers.append("missing-release-publish-handoff-completion-verification")
        verification = {}

    if set(verification) != _VERIFICATION_KEYS:
        blockers.append("unsafe-release-publish-final-authorization-verification-schema")
    if verification.get("passed") is not True:
        blockers.append("release-publish-handoff-completion-verification-not-passed")
    ...
    return ReleasePublishFinalAuthorization(
        authorized=authorized,
...
def evaluate_release_publish_final_authorization(
    verification_source: object,
    *,
    run_id: object,
    work_id: object,
    ...
    """Compatibility alias for final authorization evaluation."""
...
def _plain_mapping(value: object, blockers: list[str]) -> dict[str, Any] | None:
    ...
    if callable(to_dict):
        ...
        try:
            ...
            blockers.append("malformed-release-publish-final-authorization")
            return None
...
def _validated_mapping(value: Mapping[Any, Any], blockers: list[str]) -> dict[str, Any]:
    ...
    for key, item in value.items():
        if not isinstance(key, str):
            blockers.append("non-string-key-release-publish-final-authorization")
            continue
...
def _plain_value(value: object, blockers: list[str]) -> object:
    ...
    if callable(to_dict):
        ...
        try:
            ...
            blockers.append("unsupported-object-release-publish-final-authorization")
            return None
        if isinstance(mapped, Mapping):
            return _validated_mapping(mapped, blockers)
        blockers.append("unsupported-object-release-publish-final-authorization")
