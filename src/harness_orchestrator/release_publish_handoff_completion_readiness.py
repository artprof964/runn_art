"""Pure completion readiness boundary for Harness release publish handoff acceptance."""
...
class ReleasePublishHandoffCompletionReadiness:
    """Frozen, JSON-safe readiness data for a completed handoff acceptance."""
...
def evaluate_release_publish_handoff_completion_readiness(
    ...
    *,
    ...
    """Accept only explicit, already-passed CR-HAR-040 verification data."""
    ...
    if verification is None:
        blockers.append("missing-release-publish-handoff-acceptance-verification")
        verification = {}

    if set(verification) != _VERIFICATION_KEYS:
        blockers.append("unsafe-release-publish-handoff-completion-verification-schema")
    if verification.get("passed") is not True:
        blockers.append("release-publish-handoff-acceptance-verification-not-passed")
    ...
    expected_dependency = (
        f"release-publish-handoff-acceptance:{verification_work_id}:{package_digest[:16]}"
        if verification_work_id and package_digest
        else ""
    )
    expected_event = (
        f"release-publish-handoff-acceptance-recorded:{verification_work_id}:{package_digest[:16]}"
        if verification_work_id and package_digest
        else ""
    )
    ...
    return ReleasePublishHandoffCompletionReadiness(
        ready=ready,
...
def build_release_publish_handoff_completion_readiness(
    verification_source: object,
    *,
    run_id: object,
    work_id: object,
    ...
    """Compatibility wrapper for completion readiness evaluation."""
    ...
    if callable(to_dict):
        ...
        try:
            ...
            if not is_dataclass(value) or isinstance(value, type):
                blockers.append("malformed-release-publish-handoff-completion-readiness")
                return None
            to_dict_failed = True
...
def _validated_mapping(value: Mapping[Any, Any], blockers: list[str]) -> dict[str, Any]:
    ...
    for key, item in value.items():
        if not isinstance(key, str):
            blockers.append("non-string-key-release-publish-handoff-completion-readiness")
            continue
...
def _plain_value(value: object, blockers: list[str]) -> object:
    ...
    if callable(to_dict):
        ...
        try:
            ...
            if not is_dataclass(value) or isinstance(value, type):
                blockers.append("unsupported-object-release-publish-handoff-completion-readiness")
                return None
            to_dict_failed = True
        if not to_dict_failed and isinstance(mapped, Mapping):
            return _validated_mapping(mapped, blockers)
...
__all__ = [
    "ReleasePublishHandoffCompletionReadiness",
    "build_release_publish_handoff_completion_readiness",
