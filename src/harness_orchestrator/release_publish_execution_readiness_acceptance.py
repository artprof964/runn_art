"""Pure acceptance boundary for Harness release publish execution readiness."""
...
_MALFORMED = "malformed-release-publish-execution-readiness-acceptance"


@dataclass(frozen=True)
class ReleasePublishExecutionReadinessAcceptance:
    """Frozen, JSON-safe acceptance data for a verified execution readiness record."""
...
def evaluate_release_publish_execution_readiness_acceptance(
    ...
    *,
    ...
    """Accept only explicit, already-verified CR-HAR-049 execution readiness data."""
    ...
    if verification is None:
        blockers.append("missing-release-publish-execution-readiness-verification")
        verification = {}
    ...
    if verification.get("passed") is not True:
        blockers.append("release-publish-execution-readiness-verification-not-passed")
    ...
    expected_dependency = (
        f"release-publish-execution-readiness:{verification_work_id}:{package_digest[:16]}"
        if verification_work_id and package_digest
        else ""
    )
    expected_event = (
        f"release-publish-execution-readiness-recorded:{verification_work_id}:{package_digest[:16]}"
        if verification_work_id and package_digest
        else ""
    )
    ...
    return ReleasePublishExecutionReadinessAcceptance(
        accepted=accepted,
...
def _validated_mapping(value: Mapping[Any, Any], blockers: list[str]) -> dict[str, Any]:
    ...
    for key, item in value.items():
        if not isinstance(key, str):
            blockers.append("non-string-key-release-publish-execution-readiness-acceptance")
            continue
...
def _plain_value(value: object, blockers: list[str]) -> object:
    ...
    if callable(to_dict):
        try:
            ...
            blockers.append("unsupported-object-release-publish-execution-readiness-acceptance")
            return None
        if isinstance(mapped, Mapping):
            return _validated_mapping(mapped, blockers)
        blockers.append("unsupported-object-release-publish-execution-readiness-acceptance")
...
def _match(actual: object, expected: object, name: str, blockers: list[str]) -> None:
    if actual != expected:
        blockers.append(f"release-publish-execution-readiness-acceptance-{name}-mismatch")
