"""Pure receipt verification for Harness release publish final authorization ledgers."""
...
class ReleasePublishFinalAuthorizationReceiptVerification:
    """Frozen, JSON-safe receipt verification for final authorization ledger records."""
...
def verify_release_publish_final_authorization_receipt(
    ...
    *,
    ...
    """Validate explicit in-memory CR-HAR-045 final authorization ledger data."""
    ...
    if receipt is None:
        blockers.append("missing-release-publish-final-authorization-receipt")
        receipt = {}
    ...
    _check_recorded_and_skipped_ids(receipt, blockers)
    ...
    if len(matching_dependencies) != 1:
        blockers.append(
            ...
            else "release-publish-final-authorization-dependency-ambiguous"
        )
    if len(matching_events) != 1:
        blockers.append(
            "release-publish-final-authorization-event-missing"
            if not matching_events
            else "release-publish-final-authorization-event-ambiguous"
        )
    ...
    return ReleasePublishFinalAuthorizationReceiptVerification(
        passed=not deduped,
        blockers=deduped,
        run_id=receipt_run_id or expected_run_id_text,
        work_id=expected_work_id_text,
...
def _validate_pair(
    *,
    ...
    if dependency.get("dependency_type") != _DEPENDENCY_TYPE:
        blockers.append("release-publish-final-authorization-dependency-type-mismatch")
    if event.get("event_type") != _EVENT_TYPE:
        blockers.append("release-publish-final-authorization-event-type-mismatch")
    if dependency.get("required") is not True:
        blockers.append("release-publish-final-authorization-dependency-not-required")
    if dependency.get("status") != "authorized":
        blockers.append("release-publish-final-authorization-dependency-status-not-authorized")
    if event.get("status") != "authorized":
        blockers.append("release-publish-final-authorization-event-status-not-authorized")
    if event.get("actor") != "harness":
        blockers.append("release-publish-final-authorization-event-actor-mismatch")
    ...
    completion_source_dependency_id = _required_text(
        dependency_metadata.get("completion_source_dependency_id"),
        "completion-source-dependency-id",
        blockers,
    )
    completion_source_event_id = _required_text(
        dependency_metadata.get("completion_source_event_id"),
...
def _validate_authorization_summary(
    *,
    ...
    _match(summary.get("event_id"), source_event_id, "authorization-summary-event-id", blockers)
    _match(
...
def _validate_canonical_payload(
    *,
    ...
    _match(
        canonical.get("final_authorization"),
        authorization_summary,
        "canonical-payload-final-authorization",
        blockers,
    )
...
def _check_recorded_and_skipped_ids(receipt: Mapping[str, Any], blockers: list[str]) -> None:
    if any(key in receipt for key in ("recorded_dependency_ids", "recorded_event_ids")):
        dependency_ids = _id_tuple(
            receipt.get("recorded_dependency_ids"),
            "recorded-dependency-ids",
            blockers,
        )
        event_ids = _id_tuple(receipt.get("recorded_event_ids"), "recorded-event-ids", blockers)
        if len(dependency_ids) != 1:
            blockers.append(
                "recorded-final-authorization-dependency-id-missing"
                if not dependency_ids
                else "recorded-final-authorization-dependency-id-ambiguous"
            )
        if len(event_ids) != 1:
            blockers.append(
                "recorded-final-authorization-event-id-missing"
                if not event_ids
                else "recorded-final-authorization-event-id-ambiguous"
            )
    for key, label in (
        ("skipped_dependency_ids", "dependency-id"),
        ("skipped_event_ids", "event-id"),
        ("skipped_package_digests", "package-digest"),
    ):
        skipped = _id_tuple(receipt.get(key), f"skipped-{label}s", blockers)
        if skipped:
            blockers.append(f"skipped-final-authorization-{label}s-present")
...
def _recorded_id_match(
    ...
    if len(ids) == 1 and actual and ids[0] != actual:
        blockers.append(f"recorded-final-authorization-{kind}-id-mismatch")
...
def _plain_mapping(value: object, blockers: list[str]) -> dict[str, Any] | None:
    ...
    if callable(to_dict):
        try:
            ...
            blockers.append("malformed-release-publish-final-authorization-receipt")
            return None
...
def _validated_mapping(value: Mapping[Any, Any], blockers: list[str]) -> dict[str, Any]:
    ...
    for key, item in value.items():
        if not isinstance(key, str):
            blockers.append("non-string-key-release-publish-final-authorization-receipt")
            continue
...
def _plain_value(value: object, blockers: list[str]) -> object:
    ...
    if callable(to_dict):
        try:
            ...
            blockers.append("unsupported-object-release-publish-final-authorization-receipt")
            return None
        if isinstance(mapped, Mapping):
            return _validated_mapping(mapped, blockers)
        blockers.append("unsupported-object-release-publish-final-authorization-receipt")
...
def _match(actual: object, expected: object, name: str, blockers: list[str]) -> None:
    if actual != expected:
        blockers.append(f"release-publish-final-authorization-receipt-{name}-mismatch")
