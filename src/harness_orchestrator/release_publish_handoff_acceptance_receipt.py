"""Pure receipt verification for Harness release publish handoff acceptance ledgers."""
...
class ReleasePublishHandoffAcceptanceReceiptVerification:
    """Frozen, JSON-safe receipt verification for accepted handoff ledger records."""
...
def verify_release_publish_handoff_acceptance_receipt(
    ...
    *,
    ...
    """Validate explicit in-memory CR-HAR-039 acceptance ledger data without effects."""
    ...
    if receipt is None:
        blockers.append("missing-release-publish-handoff-acceptance-receipt")
        receipt = {}
    ...
    if len(matching_dependencies) != 1:
        blockers.append(
            ...
            else "release-publish-handoff-acceptance-dependency-ambiguous"
        )
    if len(matching_events) != 1:
        blockers.append(
            "release-publish-handoff-acceptance-event-missing"
            if not matching_events
            else "release-publish-handoff-acceptance-event-ambiguous"
        )
    ...
    return ReleasePublishHandoffAcceptanceReceiptVerification(
        passed=not deduped,
        blockers=deduped,
        run_id=receipt_run_id or expected_run_id_text,
        work_id=expected_work_id_text,
...
def _validate_pair(
    *,
    ...
    if dependency.get("dependency_type") != _DEPENDENCY_TYPE:
        blockers.append("release-publish-handoff-acceptance-dependency-type-mismatch")
    if event.get("event_type") != _EVENT_TYPE:
        blockers.append("release-publish-handoff-acceptance-event-type-mismatch")
    if dependency.get("required") is not True:
        blockers.append("release-publish-handoff-acceptance-dependency-not-required")
    if dependency.get("status") != "accepted":
        blockers.append("release-publish-handoff-acceptance-dependency-status-not-accepted")
    if event.get("status") != "accepted":
        blockers.append("release-publish-handoff-acceptance-event-status-not-accepted")
...
def _validate_acceptance_summary(
    *,
    ...
    _match(summary.get("event_id"), source_event_id, "acceptance-summary-event-id", blockers)
    _match(
...
def _validate_canonical_payload(
    *,
    ...
    _match(
        canonical.get("handoff_acceptance"),
        acceptance_summary,
        "canonical-payload-acceptance-summary",
        blockers,
    )
    ...
    if callable(to_dict):
        ...
        try:
            ...
            if not is_dataclass(value) or isinstance(value, type):
                blockers.append("malformed-release-publish-handoff-acceptance-receipt")
                return None
            to_dict_failed = True
...
def _validated_mapping(value: Mapping[Any, Any], blockers: list[str]) -> dict[str, Any]:
    ...
    for key, item in value.items():
        if not isinstance(key, str):
            blockers.append("non-string-key-release-publish-handoff-acceptance-receipt")
            continue
...
def _plain_value(value: object, blockers: list[str]) -> object:
    ...
    if callable(to_dict):
        ...
        try:
            ...
            if not is_dataclass(value) or isinstance(value, type):
                blockers.append("unsupported-object-release-publish-handoff-acceptance-receipt")
                return None
            to_dict_failed = True
        if not to_dict_failed and isinstance(mapped, Mapping):
            return _validated_mapping(mapped, blockers)
...
def _match(actual: object, expected: object, name: str, blockers: list[str]) -> None:
    if actual != expected:
        blockers.append(f"release-publish-handoff-acceptance-receipt-{name}-mismatch")
