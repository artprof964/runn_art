"""Pure authorization boundary for Harness release publish execution."""
...
_MALFORMED = "malformed-release-publish-execution-authorization"


@dataclass(frozen=True)
class ReleasePublishExecutionAuthorization:
    """Frozen, JSON-safe authorization data for verified release publish execution."""
...
def authorize_release_publish_execution(
    ...
    *,
    ...
    """Authorize only explicit, already-verified CR-HAR-052 Harness data."""
    ...
    if verification is None:
        blockers.append("missing-release-publish-execution-authorization-verification")
        verification = {}

    if set(verification) != _VERIFICATION_KEYS:
        blockers.append("unsafe-release-publish-execution-authorization-verification-schema")
    if verification.get("passed") is not True:
        blockers.append("release-publish-execution-authorization-verification-not-passed")
    ...
    source_event_id = _required_text(verification.get("source_event_id"), "source-event-id", blockers)
    ...
    final_authorization_dependency_id = _required_text(
        verification.get("final_authorization_dependency_id"),
        "final-authorization-dependency-id",
        blockers,
    )
    final_authorization_event_id = _required_text(
        verification.get("final_authorization_event_id"),
    ...
    suffix = package_digest[:16] if package_digest else ""
    expected_ids = _source_ids(verification_work_id, suffix) if verification_work_id and suffix else {}
    _match(dependency_id, expected_ids.get("dependency_id", ""), "dependency-id", blockers)
    _match(event_id, expected_ids.get("event_id", ""), "event-id", blockers)
    _match(
        source_dependency_id,
        expected_ids.get("source_dependency_id", ""),
        "source-dependency-id",
        blockers,
    )
    _match(source_event_id, expected_ids.get("source_event_id", ""), "source-event-id", blockers)
    _match(
        completion_source_dependency_id,
        expected_ids.get("completion_source_dependency_id", ""),
        "completion-source-dependency-id",
        blockers,
    )
    _match(
        completion_source_event_id,
        expected_ids.get("completion_source_event_id", ""),
        "completion-source-event-id",
        blockers,
    )
    _match(
        acceptance_source_dependency_id,
        expected_ids.get("acceptance_source_dependency_id", ""),
        "acceptance-source-dependency-id",
        blockers,
    )
    _match(
        acceptance_source_event_id,
        expected_ids.get("acceptance_source_event_id", ""),
        "acceptance-source-event-id",
        blockers,
    )
    _match(
        final_authorization_dependency_id,
        expected_ids.get("final_authorization_dependency_id", ""),
        "final-authorization-dependency-id",
        blockers,
    )
    _match(
        final_authorization_event_id,
        expected_ids.get("final_authorization_event_id", ""),
        "final-authorization-event-id",
        blockers,
    )
    ...
    receipt_summary = _required_summary(
        ...
        source_dependency_id=final_authorization_dependency_id,
        source_event_id=final_authorization_event_id,
    ...
    return ReleasePublishExecutionAuthorization(
        authorized=authorized,
        ...
        final_authorization_dependency_id=final_authorization_dependency_id,
        final_authorization_event_id=final_authorization_event_id,
        authorization_summary=summary if isinstance(summary, Mapping) else None,
...
def _required_summary(
    ...
    *,
    ...
    _match(
        ...
        final_authorization_dependency_id,
        "receipt-summary-final-authorization-dependency-id",
        blockers,
    )
    _match(
        summary.get("final_authorization_event_id"),
        final_authorization_event_id,
        "receipt-summary-final-authorization-event-id",
        blockers,
    )
...
def _plain_mapping(value: object, blockers: list[str]) -> dict[str, Any] | None:
    ...
    if is_dataclass(value) and not isinstance(value, type):
        mapped = {field.name: _plain_value(getattr(value, field.name), blockers) for field in fields(value)}
        return _validated_mapping(mapped, blockers)
    return None
...
def _validated_mapping(value: Mapping[Any, Any], blockers: list[str]) -> dict[str, Any]:
    ...
    for key, item in value.items():
        if not isinstance(key, str):
            blockers.append("non-string-key-release-publish-execution-authorization")
            continue
...
def _plain_value(value: object, blockers: list[str]) -> object:
    ...
    if callable(to_dict):
        try:
            ...
            blockers.append("unsupported-object-release-publish-execution-authorization")
            return None
        if isinstance(mapped, Mapping):
            return _validated_mapping(mapped, blockers)
        blockers.append("unsupported-object-release-publish-execution-authorization")
